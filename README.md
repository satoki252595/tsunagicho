...
2026-09-05、ユーザーが事業化着手を承認。現在は**ローカル体験版**です。公式資料に基づくガイドであり、実機確認、顧客試用、売上、支払意思は未検証。サービス名は作業名です。

- [private GitHub](https://github.com/satoki252595/tsunagicho) / [最初の増分 Issue #1](https://github.com/satoki252595/tsunagicho/issues/1)
...
HTML/CSS/JavaScriptとNode標準テストのみ。フロントの追加依存・ビルド工程はありません。WranglerとNodeはNixで固定。公開するのは`public/`のみで、企画・検証メモ・Git・顧客データは配信しません。Cloudflareの実行コード・DB・AI・決済・定期処理・外部分析ツールはありません。
...
追加費用0円、公開前確認、定期実行なし。通常のprivate repoへのpushにデプロイ連携を設定していません。実公開コマンドは許可後にだけ実行します。開発中の検証メモは`.runtime/`等に置き、顧客個人情報をコミットしません。
