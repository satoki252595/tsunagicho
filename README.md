...
2026-09-05、ユーザーが事業化着手を承認。現在は**無料の静的体験版を公開する準備中**です。2026-09-06、GitHub Pages を使うための repo 公開化と banchi サブドメインへの接続が承認されました。公開結果は[公開記録](docs/release.md)に残します。公式資料に基づくガイドであり、実機確認、顧客試用、売上、支払意思は未検証。サービス名は作業名です。

- [GitHub](https://github.com/satoki252595/tsunagicho) / [公開準備 Issue #3](https://github.com/satoki252595/tsunagicho/issues/3)
- 専用フォルダ: `/Users/satoki252595/projects/tsunagicho/`
- [事業・検証計画](docs/business.md) / [採用前の調査記録](docs/research.md) / [検証・公開候補](docs/release.md)
...
HTML/CSS/JavaScriptとNode標準テストのみ。フロントの追加依存・ビルド工程はありません。WranglerとNodeはNixで固定。Webサイトの配信対象は`public/`のみです。GitHub repo 自体はユーザー承認により文書と履歴を含めて公開するため、企画・検証記録も GitHub 上では閲覧可能になります。秘密情報・顧客個人情報は保存しません。Cloudflareの実行コード・DB・AI・決済・定期処理・外部分析ツールはありません。
...
追加費用0円、公開前確認、定期実行なし。[Pages workflow](.github/workflows/pages.yml) は main ブランチからの手動実行専用で、通常の push ではデプロイしません。公式 Actions の commit SHA を固定し、アップロード対象を `public/` に限定しています。初回公開前に確認済みの revision と DNS/HTTPS を記録します。開発中の検証メモは`.runtime/`等に置き、`.env*`、`.dev.vars*`、秘密鍵、顧客個人情報をコミットしません。
